...
| 指标 | 结果 | 预算 | 结论 |
| --- | ---: | ---: | --- |
| LCP | 1,248ms | ≤ 1,500ms | 通过 |
| CLS | 0.0003 | ≤ 0.05 | 通过 |
| TTFB | 987ms | < 1,800ms | 通过 |
| LCP 渲染延迟 | 261ms | — | 无可量化优化项 |

本次 LCP 仍是 MapLibre 署名文本，WebGL Canvas 不属于 LCP 候选。DevTools 对
渲染阻塞资源估算的 FCP/LCP 可节省时间均为 0ms，因此不为本轮数据修改 Next.js
加载策略。该域名目前没有可用 CrUX 数据，线上 Trace 仍属于实验室样本，不替代
后续真实用户 75 百分位监控。
